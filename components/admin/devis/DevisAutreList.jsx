...
export default function DevisAutreList() {
  ...
  const [syncing, setSyncing] = useState(false); // silent refresh
  ...
  const load = useCallback(
    async (silent = false) => {
      try {
        ...
        const res = await fetch(`${BACKEND}/api/devis/autre/paginated?` + params.toString(), {
          credentials: "include",
          cache: "no-store",
        });
  ...
  // أول مرة + كل تغيير في q/page/pageSize
  useEffect(() => {
    load(items.length > 0);
  }, [load]);
  ...
  // فتح inline (أسرع/أنظف من blob:)
  function viewPdfById(id) {
    window.open(`${BACKEND}/api/devis/autre/${id}/pdf`, "_blank", "noopener,noreferrer");
  }
  function viewDocByIndex(id, index) {
    window.open(`${BACKEND}/api/devis/autre/${id}/document/${index}`, "_blank", "noopener,noreferrer");
  }
