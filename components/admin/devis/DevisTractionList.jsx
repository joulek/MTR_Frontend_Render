...
const CACHE_KEY = "devisTraction.items.v1";

function cleanFilename(n = "") { return n?.startsWith?.("~$") ? "" : n || ""; }
...
export default function AdminDevisTractionPage() {
  ...
  const [loading, setLoading] = useState(true);   // skeleton يظهر فقط عند عدم وجود بيانات
  ...
  const load = useCallback(async (silent = false) => {
    try {
      ...
      const res = await fetch(`${BACKEND}/api/devis/traction/paginated?` + params.toString(), {
        credentials: "include",
        cache: "no-store",
      });
...
function viewDocByIndex(id, index) {
  window.open(`${BACKEND}/api/devis/traction/${id}/document/${index}`, "_blank", "noopener,noreferrer");
}
