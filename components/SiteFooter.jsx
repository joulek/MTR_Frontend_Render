...
export default function SiteFooter({ locale = "fr" }) {
  ...
  return (
    <footer className="relative mt-0 bg-[#F5B301] text-[#0B2239]">
      {/* grille */}
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-14 md:grid-cols-4">
        ...
        <div>
          ...
          <p className="mt-3 max-w-xs text-sm text-[#0B2239]/80">{t("brandLine")}</p>
          ...
          <div className="mt-5 flex items-center gap-3">
            <a
              ...
              className="rounded-full bg-[#0B2239]/10 p-2 text-[#0B2239] hover:bg-[#0B2239]/15"
              aria-label="Facebook"
              title="Facebook"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-full bg-[#0B2239]/10 p-2 hover:bg-[#0B2239]/15"
              title="WhatsApp : +216 98 333 883"
              aria-label="WhatsApp"
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4 text-green-600" fill="currentColor" aria-hidden="true">
        ...
        {/* Col 2 : Entreprise */}
        <div>
          <h5 className="text-lg font-semibold">{"Entreprise"}</h5>
          <ul className="mt-4 space-y-3 text-sm text-[#0B2239]/80">
            {company.map((l, i) => (
              <li key={i}>
                <Link
                  href={l.href}
                ...
                >
                  <ChevronRight className="h-4 w-4 opacity-60 text-[#0B2239]" />
                  {l.label}
                </Link>
              </li>
            ))}
          ...
          <ul className="mt-4 space-y-3 text-sm text-[#0B2239]/80">
            {contacts.map((c, i) => (
              <li key={i} className="flex flex-col">
                <span className="font-semibold text-[#0B2239]">{c.name}</span>
                <div className="flex items-center gap-2">
                  <a href={c.tel} className="hover:underline" dir="ltr" aria-label={`${c.name} téléphone`}>
                    {c.phone}
                  </a>
                </div>
              </li>
      ...
      {/* Bas de footer */}
      <div className="border-t border-[#0B2239]/15">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-5 text-sm text-[#0B2239]/80 md:flex-row">
          <p className="text-[#0B2239]">
